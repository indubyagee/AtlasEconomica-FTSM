...
CONTEXT_FILENAME = "crsp_context.parquet"
VAL_FILENAME = "crsp_val.parquet"
DATA_INPUT_PATH = "data" / "raw" / "crsp" / "dsfv2"

CONTEXT_INPUT_PATH = BASE / DATA_INPUT_PATH / CONTEXT_FILENAME
VAL_INPUT_PATH = BASE / DATA_INPUT_PATH / VAL_FILENAME
...
test_df = pd.read_parquet(VAL_INPUT_PATH)
...
OUTPUT_DIR = BASE / "data" / "crsp"
os.makedirs(OUTPUT_DIR, exist_ok=True)
print(f"[Output] Export path: {OUTPUT_DIR}")
#print(OUTPUT_DIR)
...
COMBINED_PRED_PATH = os.path.join(OUTPUT_DIR, "crsp_ftsm_results.csv")
combined_pred_df.to_csv(COMBINED_PRED_PATH, index=False)
print(f"[Output] Exported combined results to: {COMBINED_PRED_PATH}")
...
ANALYSIS_PATH = os.path.join(OUTPUT_DIR, "crsp_ftsm_analysis.csv")
analysis_df.to_csv(ANALYSIS_PATH, index=False)
print(f"[Output] Exported analysis to: {ANALYSIS_PATH}")
